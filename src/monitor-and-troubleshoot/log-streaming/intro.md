...
* Enable [log separation](../../setup-infra-platform/setup/logging-db/logs-separation-cloud/intro.md) in your environment.
...
Streaming logs from OutSystems requires the APM tool to expose an ingestion endpoint that is reachable from the public internet. Private connectivity options like VPNs, Transit gateway, or Direct Connect aren't possible. Security risks associated with publicly exposing an endpoint can be mitigated through the following measures:
...
* IP allowlisting: Firewall rules can restrict inbound traffic to the specific egress [IP addresses of the OutSystems data platform](../../setup-infra-platform/setup/network-requirements.md#log-streaming), reducing exposure to the broader internet.
...
## Benefits of log streaming { #benefits }
...
## Streaming log data { #streaming-log-data }
...
1. The log messages are converted to OTLP format and are then processed further.
...
## Supported log data { #supported-log-data }
