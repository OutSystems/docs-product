...
* **Semantic classes**: `:::start`/`:::stop` are only for workflow entry/exit points; `:::error` is only for failure outcome nodes. Use `:::process` for all component/server nodes regardless of state — show unavailability with dashed arrows (`-.->`) and a descriptive label.
...
* **Sequence diagram autonumber circles**: add `"labelBoxBkgColor": "#686E76"` and `"labelTextColor": "#FFFFFF"` to themeVariables — numbered circles render in OutSystems dark-grey with white text.
