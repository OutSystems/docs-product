...
<td>TheChange Attributes only work with an entity variable type that's populated by an Aggregate or SQL node. That data is flagged as being "original" and changes to that data by an Assign node changes this flag to "changed" and that's how the platform knows how to build the more efficient query when invoking the entity's CreateOrUpdate or Update action.</td>
</tr>
<tr>
