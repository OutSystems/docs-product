---
summary: Extensibility Configurations JSON schema for OutSystems 11 (O11) mobile apps, covering preferences, resources, icons, and access rules.
tags:
  - Android
  - Cordova
  - iOS
...
| DisableBuildCache | Android, iOS | true for MABS ≥ 13 | When set to false, MABS tries to reuse the build binaries (.apk or .ipa) to speed up creation of new native builds. Caching is applied only if the published changes [do not require a new native build](../mobile-app-update-scenarios.md). Set this preference to true for apps with plugins that invoke post-build scripts, since these scripts trigger after Gradle or Xcode finishes compiling the app resources, or when you update the files referenced in the [`resources`](#resources) property. |
