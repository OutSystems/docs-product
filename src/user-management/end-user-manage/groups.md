...
When managing large sets of app end users, it can be difficult to manage their roles individually. Instead of manually assigning roles to each end user, you can create groups of end users and assign the roles to the group.

You perform all group management operations in the [Users app](accessing-users.md).

## Create a new group

To create a new group in the Users app:
...
1. Click **Add** to assign the role to the group.

You can assign as many roles as needed to the group.

When you add an end user to the group, all the roles that are granted to the group are granted to the user. These roles appear in the user detail page with the group name under the column `Inherited from` and cannot be removed individually.
...
1. Click **Add** to add the end user as a member to the group.
...
The end user's groups can also be managed in the user details page. In the user details page, you can see a list of all the groups the user belongs to, add new groups, or remove existing ones.
...
When you remove a user from a group, all the roles granted to the user by inheritance from that group are revoked. You can still manually add a role that was granted to the user by inheritance from a group. This overrides the group inheritance - if you assign a role to an end user both through a group and individually, the role isn't revoked if the end user is removed from the group. In this scenario, the column `Inherited from` in the Roles section of the user detail page is empty.
