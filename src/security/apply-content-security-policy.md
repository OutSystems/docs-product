...
Applies to Platform Server version 11.28.0 and higher and Factory Configuration version 11.2.1 and higher.
...
* [Removing unsafe-eval](https://success.outsystems.com/support/troubleshooting/application_lifecycle/removing_unsafe_eval/)

* [Removing unsafe-inline](https://success.outsystems.com/support/troubleshooting/application_lifecycle/removing_unsafe_inline/)

## Directives reference

The table below describes the list of available directives to configure the Content Security Policy in **OutSystems**. The **Required values** column indicates the values that **LifeTime** automatically applies to the directive for the applications to work correctly. Those values can't be removed.
...
| Style-src | The domains from which applications are allowed to load styles.<br/>The following source expressions are allowed: `self`, `data:` and `*`. | `self`<br/><br/>Values added at runtime:<br/>`'unsafe-inline'` <br/>**Note**: This behavior applies to Reactive web apps only. If the **Enforce Unsafe Inline Reactive** checkbox in the [Factory Configuration](https://www.outsystems.com/forge/component-overview/25/factory-configuration) Forge component is set to **False**, this directive is removed and is **not** added at runtime. For more information, refer to [breaking changes](https://success.outsystems.com/support/release_notes/11/outsystems_11_side_effects_and_breaking_changes/#introduced-in-platform-server-11.28.0). |
